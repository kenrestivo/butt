// "$Id: Fl_My_Native_File_Chooser_MAC.mm 9685 2012-09-27 12:49:39Z manolo $"
//
...
//     Internal use only.
//
void Fl_My_Native_File_Chooser::errmsg(const char *msg) {
  _errmsg = strfree(_errmsg);
  _errmsg = strnew(msg);
}
...
void Fl_My_Native_File_Chooser::filter_value(int val) {
  _filt_value = val;
}
...
int Fl_My_Native_File_Chooser::get_saveas_basename(void) {
...
@interface FLMyopenDelegate : NSObject 
...
- (FLMyopenDelegate*)setPopup:(NSPopUpButton*)popup filter_pattern:(char**)pattern;
- (BOOL)panel:(id)sender shouldShowFilename:(NSString *)filename;
@end
@implementation FLMyopenDelegate
- (FLMyopenDelegate*)setPopup:(NSPopUpButton*)popup filter_pattern:(char**)pattern
{
  nspopup = popup;
...
@interface FLMysaveDelegate : NSObject 
#if MAC_OS_X_VERSION_MAX_ALLOWED >= MAC_OS_X_VERSION_10_6
<NSOpenSavePanelDelegate>
#endif
{
...
@end
@implementation FLMysaveDelegate
- (NSString *)panel:(id)sender userEnteredFilename:(NSString *)filename confirmed:(BOOL)okFlag
{
...
int Fl_My_Native_File_Chooser::runmodal()
{
  NSString *dir = nil;
  NSString *fname = nil;
  NSString *preset = nil;
...
//        -1 - failed; errmsg() has reason
//     
int Fl_My_Native_File_Chooser::post() {
  // INITIALIZE BROWSER
  ...
  if ( [(NSSavePanel*)_panel isKindOfClass:[NSOpenPanel class]] ) {
    ...
    if (_filt_total) {
      ...
      static FLMyopenDelegate *openDelegate = nil;
      if (openDelegate == nil) {
	// not to be ever freed
	openDelegate = [[FLMyopenDelegate alloc] init];
      }
    ...
    if ( !(_options & SAVEAS_CONFIRM) ) {
      static FLMysaveDelegate *saveDelegate = nil;
      if (saveDelegate == nil)saveDelegate = [[FLMysaveDelegate alloc] init]; // not to be ever freed
...
//
// End of "$Id: Fl_My_Native_File_Chooser_MAC.mm 9685 2012-09-27 12:49:39Z manolo $".
//
