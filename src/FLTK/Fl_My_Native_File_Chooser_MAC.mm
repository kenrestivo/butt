...
int Fl_My_Native_File_Chooser::get_saveas_basename(void) {
    ...
    if ( !(_options & SAVEAS_CONFIRM) )
    {
        ...
        //memmove(q + l, q + l + lu, strlen(q + l + lu) + 1); orig from  fltk //1.3.5
        
        // Daniel
        //fixes file picker on catalina
        if (strstr(q, UNLIKELYPREFIX))
...
int Fl_My_Native_File_Chooser::post() {
    ...
    switch (_btype) {
        ...
        case BROWSE_DIRECTORY:
            ...
            [(NSOpenPanel*)_panel setCanChooseFiles:NO]; // Daniel
            break;
        case BROWSE_SAVE_DIRECTORY:
