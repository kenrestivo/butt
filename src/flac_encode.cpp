...
static bool set_settings(flac_enc *flac)
{
    ...
    ret &= FLAC__stream_encoder_set_bits_per_sample(flac->encoder, 16);
...
static void inject_new_song_title(flac_enc *flac)
{
    ...
    {
        for (int i = 0; i < flac->vorbis_comment.data.vorbis_comment.num_comments; i++)
    ...
    flac->vorbis_comment.data.vorbis_comment.comments = 
        (FLAC__StreamMetadata_VorbisComment_Entry*)malloc(flac->vorbis_comment.data.vorbis_comment.num_comments * 
...
int flac_enc_encode_stream(flac_enc *flac, short *pcm_buf, uint8_t *enc_buf, int samples_per_chan, int channel, int new_stream)
{
    int i;
    int samples_left;
    int chunk_size;
